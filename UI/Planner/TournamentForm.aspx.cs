...
namespace GoTournamental.UI.Planner 
{
    ...
    public partial class TournamentForm : Page 
    {
        ...
        protected void AssignControlsAll() 
        {
			...
			fixtureStructure = (DropDownList)TournamentFormPanel.FindControl("FixtureStructure"); 
        ...
        protected void TournamentEditFormLoad() 
        {
			...
			if (tournament != null && !IsPostBack)
            {
                ...
                fixtureStructure.SelectedValue = EnumExtensions.GetIntValue(tournament.FixtureStructure).ToString();
        ...
        protected void ManageToolTips() 
        {
            toolTips = iToolTip.SQLSelectForWebPage("TournamentForm");
            foreach (ToolTip tip in toolTips)
            {
                foreach (Control control in TournamentFormPanel.Controls) 
                {
                    if (control is PlaceHolder)
                    {
                        if (control.ID == tip.ControlID)
                        {
                            Image icon = new Image();
                            icon.ImageUrl = "~/Images/Icons/Information.gif";
                            control.Controls.Add(icon);
                            icon.ToolTip = tip.ToolTipText;
                        }
                    }
                }
            }
